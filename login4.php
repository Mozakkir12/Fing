...
PHP;
            file_put_contents("main/db/{$db_name}.php", $dbFileContent);

            // Create 'main/config/config1.php'
...
?>
PHP;
            file_put_contents("main/config/config1.php", $configFileContent);

                        // Create 'main/db/pdo.php' with the required content
$sampleFileContent = <<<PHP
<?php
require "{$db_name}.php";
...
file_put_contents("main/db/pdo.php", $sampleFileContent);
...
if (isset($_SESSION['MYSQL_DB'])) {
    ...
    $dbFile = "main/db/{$db_name}.php";
    $configFile = "main/config/{$db_name}.php";
    $pdoCode = "main/db/pdo.php";

    if (file_exists($dbFile)) unlink($dbFile);
    if (file_exists($configFile)) unlink($configFile);
    if (file_exists($pdoCode)) unlink($pdoFile);
if (file_exists('main/db/pdo.php')) {
        unlink('main/db/pdo.php');
    }
    session_destroy();
}
?>
...
<body>
    <div class="login-container">
        ...
        <form method="POST" action="">
            ...
            <div class="form-group branch-fields" style="display: none;">
    ...
    <script>
        ...
        loginType.addEventListener('change', function () {
            branchFields.style.display = loginType.value === 'branch' ? 'block' : 'none';
        });
    </script>
</body>
</html>
